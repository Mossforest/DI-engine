...
from ding.policy import EnsembleDQNPolicy
...
def make_args():
    ...
    parser.add_argument('--model', default=5, type=int)
    args = parser.parse_args()
    return args


def make_config(args):
    breakout_ensemble_dqn_config = dict(
        exp_name=f'edqn_model{args.model}_seed{args.seed}',
        seed=args.seed,
        env=dict(
            collector_env_num=8,
            evaluator_env_num=8,
    ...
    breakout_ensemble_dqn_config = EasyDict(breakout_ensemble_dqn_config)
    main_config = breakout_ensemble_dqn_config
    breakout_ensemble_dqn_create_config = dict(
        env=dict(
            type='atari',
            import_names=['dizoo.atari.envs.atari_env'],
        ),
        env_manager=dict(type='subprocess'),        
        policy=dict(type='ensemble_dqn'),
    )
    breakout_ensemble_dqn_create_config = EasyDict(breakout_ensemble_dqn_create_config)
    create_config = breakout_ensemble_dqn_create_config
...
def main(main_config, create_config):
    ...
    with task.start(async_mode=False, ctx=OnlineRLContext()):
        ...
        policy = EnsembleDQNPolicy(cfg.policy, model=model)
        ...
        # TODO:         metric_list = ['cur_lr', 'total_loss', 'q_value', 'target_q_value', 'priority']
        # task.use(wandb_online_logger(project_name='breakout_exp', exp_name=cfg.exp_name, metric_list=metric_list))
        task.use(wandb_online_logger(project_name='asterix_exp', exp_name=cfg.exp_name))
